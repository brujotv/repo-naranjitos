...
def get_video_url( page_url , premium = False , user="" , password="", video_password="" ):
    ...
    if "shared?ci=" in page_url:
        response = httptools.downloadpage(page_url, follow_redirects=False)
        if "watch?v=" in response.headers.get("location", ""):
...
def find_videos(data):
    ...
    # https://www.youtube.com/shared?ci=iNqg0OGr0M4
    patronvideos = 'youtube.com/shared\?ci=([0-9A-z_-]+)'
    logger.info(" #"+patronvideos+"#")
    matches = re.compile(patronvideos, re.DOTALL).findall(data)

    for match in matches:
        titulo = "[YouTube]"
        url = "https://www.youtube.com/shared?ci="+match
