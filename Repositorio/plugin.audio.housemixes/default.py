...
def Index():
         addDir('Featured Mixes','http://www.house-mixes.com/djmixes/featured',2,icon,'',fanart)
         addDir('Popular Mixes','http://www.house-mixes.com/djmixes/popular',2,icon,'',fanart)
         addDir('Latest Mixes','http://www.house-mixes.com/djmixes',2,icon,'',fanart)
         addDir('Mixes by Genre','url',1,icon,'',fanart)
         addDir('Featured Tracks','http://www.house-mixes.com/djtracks/featured/1/latest',2,icon,'',fanart)
         addDir('Popular Tracks','http://www.house-mixes.com/djtracks/popular',2,icon,'',fanart)
         addDir('Latest Tracks','http://www.house-mixes.com/djtracks',2,icon,'',fanart)
...
def getmixes(url):
        link = open_url(url)
        match=re.compile('<a class="audio-image-link" href="(.+?)">.+?<img class="img-for-audio img-responsive" src="(.+?)".+?alt="(.+?)" />',re.DOTALL).findall(link)
...
def Search(url):
        ...
        if search_entered == None or len(search_entered)<1:
                ...
                url = 'http://www.house-mixes.com/searchv2?query='+ search_entered
        getmixes(url)
...
def PLAYLINK(url,name):
         ...
         if 'Media Not Found - May no longer exist' in link:
            notification('House Mixes', 'Media Not Found - May no longer exist', '5000',icon)
             ...
             playabletune=re.compile('"Mp3Url":"(.+?)"').findall(link)[0]
