...
wiz.log("[Auto Limpieza] Iniciando", xbmc.LOGNOTICE)
if AUTOCLEANUP == 'true':
	service = False
	...
	else: wiz.log("[Auto Limpeza] Siguiente limpieza %s" % AUTONEXTRUN, xbmc.LOGNOTICE)
	if service == True:
		AUTOCACHE      = wiz.getS('clearcache')
		AUTOPACKAGES   = wiz.getS('clearpackages')
		AUTOTEMP     = wiz.getS('cleartemp')
		...
		if AUTOTHUMBS == 'true': wiz.log('[Auto Limpieza] Old Thumbs: On', xbmc.LOGNOTICE); wiz.oldThumbs()
		...
		else: wiz.log('[Auto Limieza] Packages: Off', xbmc.LOGNOTICE)
else: wiz.log('[Auto Limpeiza] Cambiado off', xbmc.LOGNOTICE)
