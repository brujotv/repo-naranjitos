...
INCLUDE1X2     = ADDON.getSetting('include1x2')
INCLUDEPALANTIR = ADDON.getSetting('includepalantir')
...
def getCacheSize():
	...
	dbfiles   = [
		## TODO: fix these
		(os.path.join(ADDONDATA, 'plugin.video.palantir', 'moria.tmp')),
		(os.path.join(ADDONDATA, 'plugin.video.1x2', 'cache.db')),
	...
	cachelist = [
		...
		(os.path.join(ADDONDATA,'plugin.video.youtube','kodion')),
		(os.path.join(ADDONDATA,'script.module.metahandler','meta_cache')),
		(os.path.join(ADDONDATA,'script.module.urlresolve','cache')),
		(os.path.join(ADDONDATA,'script.module.resolveurl','cache')),
		(os.path.join(ADDONDATA,'script.module.simplecache')),
		(os.path.join(ADDONDATA,'script.module.universalscrapers')),
		(os.path.join(ADDONDATA,'plugin.video.cine')),
		(os.path.join(ADDONDATA,'script.module.streamlink.base','tmp')),
		(os.path.join(ADDONDATA,'script.module.metadatautils','animatedgifs')),
		(os.path.join(ADDONDATA,'plugin.video.itv','Images')),
		(os.path.join(ADDONDATA,'script.extendedinfo', 'images')),
		(os.path.join(ADDONDATA,'script.extendedinfo', 'TheMovieDB')),
		(os.path.join(ADDONDATA,'script.extendedinfo', 'YouTube')),
		(os.path.join(ADDONDATA,'plugin.video.tvalacarta','downloads')),
		(os.path.join(ADDONDATA,'plugin.video.alfa','downloads')),
		(os.path.join(ADDONDATA,'plugin.program.autocompletion','Google')),
		(os.path.join(ADDONDATA,'plugin.program.autocompletion', 'Bing')),
		(os.path.join(ADDONDATA,'script.module.youtube.dl','tmp')),
		(os.path.join(PROFILEADDONDATA,'plugin.video.tvalacarta','downloads')),
		(os.path.join(PROFILEADDONDATA,'plugin.program.autocompletion','Google')),
		(os.path.join(PROFILEADDONDATA,'script.extendedinfo', 'images')),
		(os.path.join(PROFILEADDONDATA,'script.extendedinfo', 'TheMovieDB')),
		(os.path.join(PROFILEADDONDATA,'script.extendedinfo', 'YouTube')),
		(os.path.join(PROFILEADDONDATA,'plugin.program.autocompletion','Bing')),
		(os.path.join(PROFILEADDONDATA,'script.module.simple.downloader')),
		(os.path.join(PROFILEADDONDATA,'plugin.video.youtube','kodion')),
		(os.path.join(PROFILEADDONDATA,'script.module.urlresolve','cache')),
		(os.path.join(PROFILEADDONDATA,'script.module.resolveurl','cache')),
		(os.path.join(PROFILEADDONDATA,'script.module.metahandler','meta_cache')),
		(os.path.join(PROFILEADDONDATA,'script.module.metadatautils','animatedgifs')),
		(os.path.join(PROFILEADDONDATA,'script.module.simplecache')),
		(os.path.join(PROFILEADDONDATA,'plugin.video.alfa','downloads')),
		(os.path.join(PROFILEADDONDATA,'script.module.universalscrapers')),
		(os.path.join(PROFILEADDONDATA,'plugin.video.cine')),
		(os.path.join(PROFILEADDONDATA,'script.module.streamlink.base','tmp')),
		(os.path.join(PROFILEADDONDATA,'script.module.youtube.dl','tmp')),
	...
	if INCLUDEVIDEO == 'true':
		...
		if INCLUDEALL == 'true': files = dbfiles
			...
			if INCLUDE1X2 == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.1x2', 'cache.db'))
			if INCLUDEPALANTIR == 'true': files.append(os.path.join(ADDONDATA, 'plugin.video.palantir', 'moria.tmp'))
		...
		else: log("Limpiar Cache: Limpiar Video Cache No disponible", xbmc.LOGNOTICE)
	return totalsize
...
def removeFolder(path):
	log("Borrar Carpetas: %s" % path, xbmc.LOGNOTICE)
	try: shutil.rmtree(path,ignore_errors=True, onerror=None)
	except: return False

def removeFile(path):
	log("Borrar Archivos: %s" % path, xbmc.LOGNOTICE)
	try:    os.remove(path)
	except: return False
			...
			for f in files:
				try: 
					...
					try:
						...
						log("Error Al borrar %s" % f, xbmc.LOGERROR)
			for d in dirs:
				total_folds += 1
				try: 
					...
					log("Error Al borrar %s" % d, xbmc.LOGERROR)
...
def log(msg, level=xbmc.LOGDEBUG):
	...
	try:
		...
		try: xbmc.log('Fallo de registro: %s' % (e), level)
		except: pass
	...
	if os.path.exists(dep):
		...
		for depends in match:
			if not 'xbmc.python' in depends:
				...
				if not DP == None: 
					DP.update("","Mirando Dependencias [COLOR yellow]%s[/COLOR] para [COLOR yellow]%s[/COLOR]" % (depends, name),"")
				if os.path.exists(dependspath):
					toggleAddon(name, 'true')
			xbmc.sleep(100)

def toggleAdult():
	do = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Te gustaria [COLOR %s]Habilitar[/COLOR] o [COLOR %s]Desahabilitar[/COLOR] los addons de adultos?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR green]SI[/COLOR][/B]", nolabel="[B][COLOR red]NO[/COLOR][/B]")
	...
	if len(found) > 0: 
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Le gustaria ver una lista de los Addons para %s?[/COLOR]" % (COLOR2, goto.replace('ing', 'ed')), yeslabel="[B][COLOR green]SI[/COLOR][/B]", nolabel="[B][COLOR red]NO[/COLOR][/B]"):
			editlist = '[CR]'.join(found)
			TextBox(ADDONTITLE, "[COLOR %s]Aqui hay una lista de los Addons para %s De contenido Adultos:[/COLOR][CR][CR][COLOR %s]%s[/COLOR]" % (COLOR1, goto.replace('ing', 'ed'), COLOR2, editlist))
		else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s][COLOR %s]%d[/COLOR] Addons para Adultos %s[/COLOR]" % (COLOR2, COLOR1, count, goto.replace('ing', 'ed')))
		forceUpdate(True)
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]No disponible[/COLOR]" % COLOR2)
...
def toggleAddon(id, value, over=None):
	...
	if os.path.exists(addonxml):
		...
		try:
			if len(tid) > 0:
				addonid = tid[0]
			if len(tservice) > 0:
				log("Tenemos un script deteniendo: %s" % match[0], xbmc.LOGDEBUG)
	...
	if 'error' in response and over == None:
		...
		DIALOG.ok(ADDONTITLE, "[COLOR %s]Error %s [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, v, id), "Verifique que la lista de Addons este actualizada e intente de nuevo.[/COLOR]")
		forceUpdate()
...
def cleanupBackup():
	...
	list = []; filelist = []
	if len(folder) == 0:
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Ubicacion de respaldo: vacio[/COLOR]" % (COLOR2))
		return
	for item in sorted(folder, key=os.path.getmtime):
		...
		if os.path.isdir(item): 
			...
			list.append(base)
	list = ['--- Eliminar todos los elementos ---'] + list
	selected = DIALOG.select("%s: Seleccione los elementos para eliminar de 'MyBuilds'." % ADDONTITLE, list)
	
	if selected == -1:
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Limpieza Cancelada![/COLOR]" % COLOR2)
	elif selected == 0: 
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Le gustaria limpiar todos los articulos en su carpeta 'My_Builds'?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, MYBUILDS), yeslabel="[B][COLOR green]SI[/COLOR][/B]", nolabel="[B][COLOR red]NO[/COLOR][/B]"):
			clearedfiles, clearedfolders = cleanHouse(xbmc.translatePath(MYBUILDS))
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Borrar Archivos: [COLOR %s]%s[/COLOR] / Carpetas:[/COLOR] [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, clearedfiles, COLOR1, clearedfolders))
		else:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Limpiar Cancelado![/COLOR]" % COLOR2)
	else:
		path = filelist[selected-1]; passed = False
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Te gustaria eliminar [COLOR %s]%s[/COLOR] de la carpeta 'My_Builds' ?[/COLOR]" % (COLOR2, COLOR1, list[selected]), "[COLOR %s]%s[/COLOR]" % (COLOR1, path), yeslabel="[B][COLOR green]SI[/COLOR][/B]", nolabel="[B][COLOR red]NO[/COLOR][/B]"):
			if os.path.isfile(path): 
				try:
					os.remove(path)
					passed = True
				except:
					log("No se puede eliminar: %s" % path)
			else:
				cleanHouse(path)
				try:
					shutil.rmtree(path)
					...
					log("Error al borrar %s" % path, xbmc.LOGNOTICE)
			if passed: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]%s Eliminar![/COLOR]" % (COLOR2, list[selected]))
			else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Error Al eliminar %s![/COLOR]" % (COLOR2, list[selected]))
		else:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Limpieza Cancelada![/COLOR]" % COLOR2)
...
def forceUpdate(silent=False):
	...
	if silent == False: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Forzar actualizaciones[/COLOR]' % COLOR2)

def convertSpecial(url, over=False):
	total = fileCount(url); start = 0
	DP.create(ADDONTITLE, "[COLOR %s]Cambiando las rutas fisicas a especiales" % COLOR2, "", "Espere[/COLOR]")
	for root, dirs, files in os.walk(url):
		for file in files:
			start += 1
			perc = int(percentage(start, total))
			if file.endswith(".xml") or file.endswith(".hash") or file.endswith("properies"):
				DP.update(perc, "[COLOR %s]Buscando: [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, root.replace(HOME, '')), "[COLOR %s]%s[/COLOR]" % (COLOR1, file), "Espere[/COLOR]")
				...
				f.close()
	DP.close()
	log("[Convertir rutas a especiales] Completada", xbmc.LOGNOTICE)
	if over == False: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Convertir rutas a especiales: Completada![/COLOR]" % COLOR2)
...
def clearCrash():  
	...
	for file in glob.glob(os.path.join(LOG, '*crashlog*.*')):
		files.append(file)
	if len(files) > 0:
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Desea eliminar los registros de Crash?' % COLOR2, '[COLOR %s]%s[/COLOR] Archivos encontrados[/COLOR]' % (COLOR1, len(files)), yeslabel="[B][COLOR green]SI[/COLOR][/B]", nolabel="[B][COLOR red]NO[/COLOR][/B]"):
			for f in files:
				os.remove(f)
			LogNotify('[COLOR %s]Limpiar Crash Logs[/COLOR]' % COLOR1, '[COLOR %s]%s Crash Logs Eliminado[/COLOR]' % (COLOR2, len(files)))
		else: LogNotify('[COLOR %s]%s[/COLOR]' % (COLOR1, ADDONTITLE), '[COLOR %s]Limpiar Crash Logs Cancelado[/COLOR]' % COLOR2)
	else: LogNotify('[COLOR %s]Limpiar Crash Logs[/COLOR]' % COLOR1, '[COLOR %s]No localizado[/COLOR]' % COLOR2)
...
def asciiCheck(use=None, over=False):
			...
			try:
				...
				if yes:
					try: 
						...
						afiles.write('\n')
						f1 += 1
						log("[ASCII Check] Archivos Eliminados: %s " % badfile, xbmc.LOGERROR)
					except:
						for chunk in chunks(badfile, 75):
							afails.write(chunk+'\n')
						afails.write('\n')
						f2 += 1
						log("[ASCII Check] Archivo Fallido: %s " % badfile, xbmc.LOGERROR)
				else:
					for chunk in chunks(badfile, 75):
						afiles.write(chunk+'\n')
					afiles.write('\n')
	...
	if total > 0:
		...
		if yes:
			if use:
				LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]ASCII Check: %s Borrado / %s Fallido.[/COLOR]" % (COLOR2, f1, f2))
			else:
				TextBox(ADDONTITLE, "[COLOR yellow][B]%s Archivo Fallido:[/B][/COLOR]\n %s\n\n[COLOR yellow][B]%s Archivo Fallido:[B][/COLOR]\n %s" % (f1, msg, f2, msg2))
		else: 
			...
			for base, dirs, files in os.walk(HOME):
				...
				for file in files:
					try:
						...
						elif os.path.join('addons', 'temp') in fn: log("[Back Up] Type = '%s': Ignore %s" % (type, file), xbmc.LOGNOTICE); continue
...
def whiteList(do):
	...
	if   do == 'edit':
		...
		list = whiteList('read')
		if len(list) > 0:
			msg = "Aqui hay una lista de sus elementos de la lista blanca, estos elementos (junto con las dependencias) no se eliminaran al realizar un nuevo inicio o los datos de usuario sobrescritos en una instalacion del Addon.[CR][CR]"
			for item in list:
				try: name, id, fold = item
				except Exception, e: log(str(e))
		...
		try:
			for root, dirs, files in os.walk(PACKAGES):
				...
				if file_count > 0:
					...
					if over: yes=1
					else: yes=DIALOG.yesno("[COLOR %s]Borrar Package" % COLOR2, "[COLOR %s]%s[/COLOR] Archivos / [COLOR %s]%s[/COLOR] Total." % (COLOR1, str(file_count), COLOR1, size), "Quieres borrarlos?[/COLOR]", nolabel='[B][COLOR red]NO[/COLOR][/B]',yeslabel='[B][COLOR green]SI[/COLOR][/B]')
					if yes:
						for f in files: os.unlink(os.path.join(root, f))
						for d in dirs: shutil.rmtree(os.path.join(root, d))
						LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Limpiar Packages: OK![/COLOR]' % COLOR2)
				else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Limpiar Packages: OK![/COLOR]' % COLOR2)
		except Exception, e:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Limpiar Packages: Error![/COLOR]' % COLOR2)
				...
				if lastedit <= start:
					if os.path.isfile(file):
						...
						try:
							...
							log("Error al Borrar %s: %s" % (file, str(e), xbmc.LOGERROR))
			if file_count > 0: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Limpiar Packages: OK: %s[/COLOR]' % (COLOR2, convertSize(cleanupsize)))
			else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Limpiar Packages: OK![/COLOR]' % COLOR2)
		except Exception, e:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Limpiar Packages: Error![/COLOR]' % COLOR2)
			log("Limpiar Packages Error: %s" % str(e), xbmc.LOGERROR)
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Limpiar Packages: OK![/COLOR]' % COLOR2)

def clearTemp(over=None):
	if os.path.exists(TEMP):
		try:
			for root, dirs, files in os.walk(TEMP):
				...
				file_count += len(files)
				if file_count > 0:
					size = convertSize(getSize(TEMP))
					if over: yes=1
					else: yes=DIALOG.yesno("[COLOR %s]Borrar Archivos Temp" % COLOR2, "[COLOR %s]%s[/COLOR] Archivos / [COLOR %s]%s[/COLOR] Total." % (COLOR1, str(file_count), COLOR1, size), "Quieres borrarlos?[/COLOR]", nolabel='[B][COLOR red]NO[/COLOR][/B]',yeslabel='[B][COLOR green]SI[/COLOR][/B]')
					if yes:
						for f in files: os.unlink(os.path.join(root, f))
						for d in dirs: shutil.rmtree(os.path.join(root, d))
						LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Limpiar Archivos Temp: OK![/COLOR]' % COLOR2)
				else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Limpiar Archivos Temp: OK![/COLOR]' % COLOR2)
		except Exception, e:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Limpiar Archivos Temp: Error![/COLOR]' % COLOR2)
			log("Clear Temp Error: %s" % str(e), xbmc.LOGERROR)
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Limpiar Archivos Temp: OK![/COLOR]' % COLOR2)

def clearTempStartup():
	start = datetime.utcnow() - timedelta(minutes=3)
	file_count = 0; cleanupsize = 0
	if os.path.exists(TEMP):
				...
				if lastedit <= start:
					if os.path.isfile(file):
						...
						try:
							...
							log("Error al Borrar %s: %s" % (file, str(e), xbmc.LOGERROR))
			if file_count > 0: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Limpiar Archivos Temp: OK: %s[/COLOR]' % (COLOR2, convertSize(cleanupsize)))
			else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Limpiar Archivos Temp: OK![/COLOR]' % COLOR2)
		except Exception, e:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Limpiar Archivos Temp: Error![/COLOR]' % COLOR2)
			log("Limpiar Packages Error: %s" % str(e), xbmc.LOGERROR)
					...
					for f in files:
						if not f in LOGFILES:
							try:
								...
								pass
						else: log('Ignorar Log File: %s' % f, xbmc.LOGNOTICE)
					for d in dirs:
						try:
							shutil.rmtree(os.path.join(root, d))
							delfiles += 1
							log("[OK] Limpiar %s Archivos de %s" % (str(file_count), os.path.join(item,d)), xbmc.LOGNOTICE)
						except:
							log("[Error] borrar el cache en: %s" % os.path.join(item,d), xbmc.LOGNOTICE)
		else:
			for root, dirs, files in os.walk(item):
				dirs[:] = [d for d in dirs if d not in excludes]
				for d in dirs:
					if not str(d.lower()).find('cache') == -1:
						try:
							shutil.rmtree(os.path.join(root, d))
							delfiles += 1
							log("[OK] borrados %s " % os.path.join(root,d), xbmc.LOGNOTICE)
						except:
							log("[Error] borrar el cache en: %s" % os.path.join(item,d), xbmc.LOGNOTICE)
			...
			for item in files:
				if os.path.exists(item):
					...
					try:
						...
						log("Error de conexion DB: %s" % str(e), xbmc.LOGERROR)
						continue
					if 'Database' in item:
						try:
							...
							textdb.commit()
							textexe.close()
							log("[OK] Borrados %s" % item, xbmc.LOGNOTICE)
						except Exception, e:
							log("[Error] al borrara %s: %s" % (item, str(e)), xbmc.LOGNOTICE)
					else:
						...
						for table in textexe.fetchall():
							try:
								...
								textexe.execute("VACUUM")
								textdb.commit()
								log("[OK] Borrados %s en %s" % (table, item), xbmc.LOGNOTICE)
							except Exception, e:
								log("[Error] al borrar %s en %s: %s" % (table, item, str(e)), xbmc.LOGNOTICE)
						textexe.close()
		else: log("Limpiar Cache: Limpiar Video Cache No disponible", xbmc.LOGNOTICE)
	LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Limpiar Cache: Borrar %s Archivos[/COLOR]' % (COLOR2, delfiles))

def checkSources():
	if not os.path.exists(SOURCES):
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]No Sources.xml No disponible![/COLOR]" % COLOR2)
		return False
	x      = 0
	bad    = []
	remove = []
		...
		if len(bad) > 0:
			...
			if choice == 1:
				...
				for name, path, sharing, working in bad: 
					...
					else: log("Source %s no fue eliminado" % name, xbmc.LOGNOTICE)
			if len(remove) > 0:
			...
			else: log("No hay malas fuentes para eliminar.", xbmc.LOGNOTICE)
...
def reloadFix(default=None):
	DIALOG.ok(ADDONTITLE, "[COLOR %s]ADVERTENCIA: A veces, Kodi se cuelgua. Fuerza el cierre para terminar la instalacion.[/COLOR]" % COLOR2)
	if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
	if default == None:
		lookandFeelData('save')
	redoThumbs()
...
def oldThumbs():
	...
	if len(images) > 0: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Limpiar Thumbs: %s Archivos / %s MB[/COLOR]!' % (COLOR2, str(len(images)), removed))
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Limpiar Thumbs: OK![/COLOR]' % COLOR2)
